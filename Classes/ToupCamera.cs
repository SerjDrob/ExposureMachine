...
class ToupCamera : IVideoCapture
{
    static ToupCamera()
    {
        try
        {
            ...
            {
                _devices = new string[arr.Length];
                _devices = arr.Select(a => a.id).ToArray();
            }
        }
        catch (Exception e)
        {
    ...
    public Dictionary<int, (string, string[])> AvaliableVideoCaptureDevices => throw new NotImplementedException();

    public bool IsVideoCaptureConnected => throw new NotImplementedException();

    public string VideoCaptureMessage => throw new NotImplementedException();
    ...
    public int GetVideoCapureDevicesCount()
    {
        throw new NotImplementedException();
    }
    ...
    private void OnEventImage()
    {
        if (bmp_ is not null)
        {
            ...
            try
            {
                ...
                bitmap.EndInit();
                bitmap.Freeze();
                OnBitmapChanged?.Invoke(null, new VideoCaptureEventArgs(bitmap, DeviceIndex.ToString()));
            }
            catch (Exception ex)
            {
            }
    ...
    public void StartCamera(int index, int capabilitiesInd = 0)
    {
        ...
        if (cam_ is null)
        {
            ...
            cam_ = Toupcam.Open(_devices[index]);
